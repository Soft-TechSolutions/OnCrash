...
import com.softtech.onerror.activity.DefaultErrorActivity;
...
public final class CustomActivityOnCrash {
    ...
    private static final String EXTRA_CONFIG = "com.softtech.onerror.EXTRA_CONFIG";
    private static final String EXTRA_STACK_TRACE = "com.softtech.onerror.EXTRA_STACK_TRACE";
    private static final String EXTRA_ACTIVITY_LOG = "com.softtech.onerror.EXTRA_ACTIVITY_LOG";

    //General constants
    private static final String INTENT_ACTION_ERROR_ACTIVITY = "com.softtech.onerror.ERROR";
    private static final String INTENT_ACTION_RESTART_ACTIVITY = "com.softtech.onerror.RESTART";
    private static final String CAOC_HANDLER_PACKAGE_NAME = "com.softtech.onerror";
    ...
    private static ExceptionConfig config = new ExceptionConfig();
                        ...
                        public void uncaughtException(@NonNull Thread thread, @NonNull final Throwable throwable) {
                            if (config.isEnabled()) {
                                ...
                                if (hasCrashedInTheLastSeconds(application)) {
                                    ...
                                    } else if (config.getBackgroundMode() == ExceptionConfig.BACKGROUND_MODE_SHOW_CUSTOM || !isInBackground
                                    ...
                                    } else if (config.getBackgroundMode() == ExceptionConfig.BACKGROUND_MODE_CRASH) {
                                        if (oldHandler != null) {
                                            oldHandler.uncaughtException(thread, throwable);
                                            return;
                                        }
     ...
     */
    @Nullable
    public static ExceptionConfig getConfigFromIntent(@NonNull Intent intent) {
        ExceptionConfig config = (ExceptionConfig) intent.getSerializableExtra(CustomActivityOnCrash.EXTRA_CONFIG);
     ...
     */
    public static void restartApplicationWithIntent(@NonNull Activity activity, @NonNull Intent intent, @NonNull ExceptionConfig config) {
    ...
    public static void restartApplication(@NonNull Activity activity, @NonNull ExceptionConfig config) {
     ...
     * @param config   The config object as obtained by calling getConfigFromIntent.
     */
    public static void closeApplication(@NonNull Activity activity, @NonNull ExceptionConfig config) {
        if (config.getEventListener() != null) {
    ...
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    @NonNull
    public static ExceptionConfig getConfig() {
        return config;
    }
     ...
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public static void setConfig(@NonNull ExceptionConfig config) {
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="com.softtech.onerror.RESTART" />,
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="com.softtech.onerror.RESTART" />,
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="com.softtech.onerror.ERROR" />,
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="com.softtech.onerror.ERROR" />,
     * If there is no activity with that intent filter, this returns null.
     *
     * @param context A valid context. Must not be null.
     * @return A valid activity class, or null if no suitable one is found
